...
void Formula::saveAsPostfix(const std::string infix)
{
	...
	postfix = new Element[size];						//수와 연산자를 저장할 메모리 할당
...
//후위표기식 계산
int Formula::calculate()
{
	Stack<int> resultStack;
	for (int i = 0; i < size; i++)
	{
		if (!postfix[i].getIsOperator())								//값이 피연산자이면(숫자이면)
		{
			resultStack.push(postfix[i].getIntValue());		//값을 int형으로 stack에 push
		}
		else													//값이 연산자이면
		{
			int result;
			if (postfix[i].getStrValue()[0] == '~' || postfix[i].getStrValue()[0] == '#')	//단항연산자 계산
			{
				int C = resultStack.pop();
				result = processUnaryOperator(C, postfix[i].getStrValue()[0]);
			}
			else																			//일반연산자 계산
			{
				int A = resultStack.pop();
				int B = resultStack.pop();
				result = processOperator(B, A, postfix[i].getStrValue()[0]);
